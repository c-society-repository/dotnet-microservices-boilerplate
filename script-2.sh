...
while true; do
  ...
  echo "$MESSAGES" | jq -c '.[]' | while read -r message; do
    ...
    ATTRIBUTES=$(echo "$message" | jq -r '.message.attributes // {}')
    ATTRIBUTES_ARGS=""

    # Montar os atributos (se houver)
    if [ "$ATTRIBUTES" != "{}" ]; then
      for key in $(echo "$ATTRIBUTES" | jq -r 'keys[]'); do
        value=$(echo "$ATTRIBUTES" | jq -r --arg key "$key" '.[$key]')
        ATTRIBUTES_ARGS="$ATTRIBUTES_ARGS --attribute=$key=$value"
      done
    fi
